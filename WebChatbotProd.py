...
# Prompt user to enter their SERP API key
serp_api_key = st.text_input("Enter your SERP API Key to continue:", type="password")
if not serp_api_key:
    st.warning("[Get your SERP API Key for Free here](https://serpapi.com/users/welcome)")
    st.stop()
...
model = OpenAIModel(api_key=api_key, model="gpt-3.5-turbo", serp_api_key=serp_api_key)
